...
MC_rep = function(i){
  ...
  result_vect = c(little_test(X, alpha),
                  ...
                  MCAR_corr_test(X, alpha, B = 99))
  return(result_vect)
}

varList = varlist(
   n.sim = list(type = "N", expr = quote(N[sim]), value = 5000),
   n = list(type = "grid", value = c(200, 400, 1000)))
...
for(t1 in seq(t2-t3, pi - t3, length.out = 3)){
  ...
  options(mc.cores=2)
  res = doClusterApply(1:M, doOne=MC_rep, monitor=TRUE)
