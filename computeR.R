...
computeRalpha = function(patterns=list(), SigmaS=list(), alpha) {
...
computeRc = function(patterns=list(), SigmaS=list(), c) {
  ...
  for(j in 1:card_patterns){
    card_S = length(patterns[[j]])
    C[[card_patterns+1+j]] = matrix(0, card_S, card_S)
  }
  ...
  for(j in 1:card_patterns){
    card_S = length(patterns[[j]])
    Ablank[[card_patterns+1+j]] = matrix(0, card_S, card_S)
  }
  ...
  for(k in 1:card_patterns){
    ...
    for (i in 1:card_S){
      for (j in i:card_S){
        b[ind + (i-1)*(card_S+1)-(i-1)*i/2 + j+1-i] = c*as.numeric(i == j)
        
        e_i = rep(0, card_S); e_i[i]=1
        e_j = rep(0, card_S); e_j[j]=1
        A[[ind + (i-1)*(card_S+1)-(i-1)*i/2 + j+1-i]] = Ablank
        A[[ind + (i-1)*(card_S+1)-(i-1)*i/2 + j+1-i]][[card_patterns+k+1]] = 
          -(e_i%*%t(e_j)/2 + e_j%*%t(e_i)/2)
  ...
  sizes = numeric(length = 1+2*card_patterns)
  sizes[1] = d; ind = 2
  for (S in patterns){
    sizes[ind] = length(S)
    ind = ind+1
  ...
  K = list(type=rep("s",1+2*card_patterns),size=sizes) # Variables are the PSD matrices Sigma,Z_{1,2},...,Z_{d,1}
...
nuclearNorm = function(XS){
  sum = 0 
  for (i in 1:length(XS)){
    sum = sum + sum(diag(XS[[i]]))
  }
  return(sum)
}

innerS = function(XS, YS){
  sum = 0
  for (i in 1:length(XS)){
    sum = sum + sum(diag(XS[[i]]%*%YS[[i]]))
  }
  return(sum)
}
...
for (xxx in 1:10000){
  ...
  tmp = computeR(list(c(1,2),c(2,3), c(1,3)), SigmaS = SigmaS)
  tmp$R
  
  theta_1 = acos(SigmaS[[1]][1,2])
  theta_2 = acos(SigmaS[[2]][1,2])
  theta_3 = acos(SigmaS[[3]][1,2])
  
  while(theta_2 > pi/2 || theta_3 > pi/2 || theta_1 < max(theta_2, theta_3)){
    d = 3
    SigmaS=list() #Random 2x2 correlation matrices (necessarily consistent)
    for(j in 1:d){
      x=runif(2,min=-1,max=1); y=runif(2,min=-1,max=1); SigmaS[[j]]=cov2cor(x%*%t(x) + y%*%t(y))
  ...
  phi_1 = acos(tmp$Sigma[1,2]/(1-tmp$R))
  # (cos(phi_1) - cos(theta_1))/(1 + cos(phi_1))
  phi_2 = acos(tmp$Sigma[2,3]/(1-tmp$R))
  phi_3 = acos(tmp$Sigma[1,3]/(1-tmp$R))
  
  # 2*sin(0.5*(phi_1+theta_1))*sin(0.5*(theta_1 - theta_2 - theta_3))/(1 + cos(phi_1))
  beta = phi_2 + phi_3 - theta_2 - theta_3
  Y = c(Y, beta)
  X = c(X, tmp$R)
}
...
plot(X,Y, type="p")
    ...
    B = matrix(c(1, SigmaS[[1]][1,2], SigmaS[[3]][1,2],
                 SigmaS[[1]][1,2], 1, SigmaS[[2]][1,2], 
                 SigmaS[[3]][1,2], SigmaS[[2]][1,2], 1), nrow = 3)
    c = min(eigen(B)$values)
    
    beta = 1000
    old = computeR(list(c(1,2),c(2,3), c(1,3)), SigmaS = SigmaS)$R
    tmp = computeRalpha(list(c(1,2),c(2,3), c(1,3)), SigmaS = SigmaS, beta)
    neww = tmp$R
    z = tmp$z
    ...
    SigmaS[[1]]+as.numeric(z)*diag(2)
    tmp$SigmaSprime[[1]]+tmp$Sigma[c(1,2),c(1,2)]
  ...
  old = computeR(patterns = list(c(2,3,4,5),c(1,3,4,5),c(1,2,4,5),c(1,2,3,5),c(1,2,3,4)), SigmaS = SigmaS)$R
  new = computeRc(patterns = list(c(2,3,4,5),c(1,3,4,5),c(1,2,4,5),c(1,2,3,5),c(1,2,3,4)), SigmaS = SigmaS, 0.1)$R
  neww = computeRalpha(patterns = list(c(2,3,4,5),c(1,3,4,5),c(1,2,4,5),c(1,2,3,5),c(1,2,3,4)), SigmaS = SigmaS, 55)$R
  z = computeRalpha(patterns = list(c(2,3,4,5),c(1,3,4,5),c(1,2,4,5),c(1,2,3,5),c(1,2,3,4)), SigmaS = SigmaS, 55)$z
...
### Miss methods
  ...
  data = rMvdc(n, P)
  X = delete_MAR_rank(data, 0.3, c(1,2), cols_ctrl = c(3, 3))
  result = get_SigmaS(X)
  
  old = computeR(result$patterns, result$SigmaS)$R
  neww = computeRalpha(result$patterns, result$SigmaS, 10)$R
